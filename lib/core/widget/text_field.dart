...
class TextFieldWidget extends StatelessWidget {
  ...
  final Color? color,borderColor,cursorColor,focusedBorder,enabledBorder,hintColor,labelColor;
  final TextStyle? style;
  final TextAlign? textAlign;
  final EdgeInsets? contentPadding;
  const TextFieldWidget({this.maxLines,this.hintText,this.cursorColor,this.hintColor,this.required,
    required this.controller,this.height,this.width,this.style,this.focusedBorder,
    this.enabledBorder,this.isLabel = false,this.label,this.labelColor,
    ...
    this.onTextTap,
    this.validator,this.verticalPadding,super.key, this.minLines,this.contentPadding, this.elevation, this.readOnlyOnly =false});
  @override
          ...
          children: [
            ...
            if (label != null)
              PositionedDirectional(
                top: -1.h,start: 2.w,
                child: Container(color:readOnly? Color(0xffF0F1F2): labelColor?? AppColor.backgroundColor,
  ...
  InputDecoration inputDecoration(){
    return InputDecoration(
      ...
      fillColor: readOnly?  Color(0xffF0F1F2): (color??AppColor.backgroundColor),
      filled: true,
      ...
      border: otp ?UnderlineInputBorder(borderSide: BorderSide(color: AppColor.defaultColor),):
      border(borderRadius: borderRadius,color: borderColor??AppColor.smallWhite,),
      // disabledBorder:border(borderRadius: borderRadius,color: borderColor,otp: otp),
      disabledBorder:otp ?UnderlineInputBorder(borderSide: BorderSide(color: AppColor.defaultColor),):
      border(borderRadius: borderRadius,color: borderColor??AppColor.smallWhite,),
      // focusedBorder: border(borderRadius: borderRadius,color: focusedBorder??borderColor,
      //     borderWidth: focusedBorder==null?0:3,otp: otp),
      focusedBorder: otp ?UnderlineInputBorder(borderSide: BorderSide(color: AppColor.defaultColor),):
      border(borderRadius: borderRadius,color: borderColor??AppColor.smallWhite,),
      enabledBorder: otp ?UnderlineInputBorder(borderSide: BorderSide(color: AppColor.defaultColor),):
      border(borderRadius: borderRadius,color: borderColor??AppColor.smallWhite,),
      // enabledBorder: border(borderRadius: borderRadius,color: enabledBorder??borderColor,otp: otp),
      errorBorder: otp ?UnderlineInputBorder(borderSide: BorderSide(color: AppColor.defaultColor),):
      ...
      prefixIcon: prefix,hintStyle: TextStyleClass.smallStyle(color: hintColor),
      errorStyle: TextStyleClass.smallStyle(color: Colors.red),
      contentPadding: contentPadding??EdgeInsets.symmetric(horizontal: 3.w,vertical: 2.h),
  ...
  static InputBorder border({Color? color,double? borderRadius,double? borderWidth,bool otp = false}){
    if(otp){
      return UnderlineInputBorder(borderRadius: BorderRadius.circular(borderRadius??10),
        borderSide: BorderSide(color: color??Color(0xff8F8C8C),width: borderWidth??1),);
    }
    return OutlineInputBorder(borderRadius: BorderRadius.circular(borderRadius??10),
      borderSide: BorderSide(color: color??Color(0xff8F8C8C),width: borderWidth??1),);
  }
